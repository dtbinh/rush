...
bool Voronoi::Generate( const Vec2* pSites, int numPoints, const Frame& bounds, float minDist )
{
	cleanup();
	cleanupEdges();
	int i;
	...
	for(i = 0; i< nsites; i++)
	{
		sites[i].coord.x = pSites[i].x;
		sites[i].coord.y = pSites[i].y;
		sites[i].sitenbr = i;
		sites[i].refcnt = 0;

		if(pSites[i].x < xmin)
			xmin = pSites[i].x;
		else if(pSites[i].x > xmax)
			xmax = pSites[i].x;

		if(pSites[i].y < ymin)
			ymin = pSites[i].y;
		else if(pSites[i].y > ymax)
			ymax = pSites[i].y;
