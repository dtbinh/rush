...
void JRenderServer::InitGlobalConstants()
{
...
} // JRenderServer::InitGlobalConstants

void JRenderServer::SetGlobalConstants()
{
...
} // JRenderServer::SetGlobalConstants
...
bool JRenderServer::ResizeBackBuffer( int w, int h )
{
    if (m_pDevice == NULL || 
...
void* JRenderServer::GetRenderSurface()
{
    return m_pBackBuffer;
}

void JRenderServer::ResetTime()
{
    m_Time.Start();
} 
...
void JRenderServer::ReloadResources()
{
    //  reload shaders
...
int JRenderServer::GetConstantID( int shaderID, const char* name )
{
    ...
    return res;
} // JRenderServer::GetConstantID

bool JRenderServer::SetConstant( int shaderID, int constID, const Mat4& val )
{
    ...
    return (hRes == S_OK);
} // JRenderServer::SetConstant

bool JRenderServer::SetConstant( int shaderID, int constID, float x, float y, float z, float w )
{
    if (shaderID < 0 || shaderID >= m_Shaders.size() || constID == NULL) return false;
    const ShaderInstance& shader = m_Shaders[shaderID];
...
bool JRenderServer::SetConstant( int shaderID, int constID, float val )
{
    if (shaderID < 0 || shaderID >= m_Shaders.size() || constID == NULL) return false;
    const ShaderInstance& shader = m_Shaders[shaderID];
...
void JRenderServer::SetShader( int shID )
{
    ...
    {
      UINT nPasses = 0;
      DWORD flags = D3DXFX_DONOTSAVESTATE | D3DXFX_DONOTSAVESAMPLERSTATE | D3DXFX_DONOTSAVESHADERSTATE;
...
int JRenderServer::CreateVB( const char* name, int numBytes, bool bStatic )
{
    ...
    DWORD fvf = 0;//CreateFVF( GetStockVDecl( VertexType_XYZWDSUV ) );
