...
#define FORCE_RELEASE(p) { if(p) { while((p)->Release()); (p)=NULL; } }

//****************************************************************************/
/*  JRenderServerDX9
//****************************************************************************/
decl_class(JRenderServerDX9);
JRenderServerDX9::JRenderServerDX9()
{
...
void JRenderServerDX9::ResetStats()
{
...
void JRenderServerDX9::InitGlobalConstants()
{
...
} // JRenderServerDX9::InitGlobalConstants

void JRenderServerDX9::SetGlobalConstants()
{
...
} // JRenderServerDX9::SetGlobalConstants

void JRenderServerDX9::Init()
{
    SetName( "render" );
    ...
    */
	...
	//  check for NvPerfHUD presence and enable it, if present
	UINT adapterID = D3DADAPTER_DEFAULT;
	for (UINT i = 0; i < m_pD3D->GetAdapterCount(); i++)
	{
		D3DADAPTER_IDENTIFIER9 adid;
		HRESULT res = m_pD3D->GetAdapterIdentifier( i, 0, &adid );
		if (strstr( adid.Description, "PerfHUD" ) != 0)
		{
			adapterID = i;
			devType	  = D3DDEVTYPE_REF;
			break;
...
} // JRenderServerDX9::Init

bool JRenderServerDX9::IsFullScreen() const
{
    return m_PresParam.Windowed ? false : true;
}

void JRenderServerDX9::SetFullScreen( bool bFullScreen )
{
...
bool JRenderServerDX9::ResizeBackBuffer( int w, int h )
{
    if (m_pDevice == NULL || 
...
void* JRenderServerDX9::GetRenderSurface()
{
    return m_pBackBuffer;
}

void JRenderServerDX9::ResetTime()
{
    m_Time.Start();
} 

bool JRenderServerDX9::StartFrame()
{
    ...
    return true;
} // JRenderServerDX9::StartFrame

void JRenderServerDX9::EndFrame()
{
    if (!m_pDevice) return;
    m_CurFrame++;
...
} // JRenderServerDX9::EndFrame

void JRenderServerDX9::ReloadResources()
{
    //  reload shaders
    ...
    D3DXCreateEffectPool( &m_pEffectPool );
    InitGlobalConstants();
} // JRenderServerDX9::ReloadResources
    
void JRenderServerDX9::ClearViewport( uint32_t color )
{
    if (!m_pDevice) return;
    m_pDevice->Clear( 0, NULL, D3DCLEAR_TARGET, color, 1.0, 0 );
} // JRenderServerDX9::ClearViewport

void JRenderServerDX9::ClearDepthStencil( bool bDepth, bool bStencil )
{
    if (!m_pDevice) return;
    uint32_t flags = 0;
...
}  // JRenderServerDX9::ClearDepthBuffer

void JRenderServerDX9::SetViewport( const Frame& rc )
{
    if (!m_pDevice) return;
...
} // JRenderServerDX9::SetViewport

bool JRenderServerDX9::Shut()
{
    if (!m_bInited) 
    {
        return false;
    ...
    m_bInited = false;
    return true;
} // JRenderServerDX9::Shut

void JRenderServerDX9::SetViewTM( const Mat4& tm )
{
    m_ViewTM = tm;
    m_pDevice->SetTransform( D3DTS_VIEW, (D3DMATRIX*)&tm );
} // JRenderServerDX9::SetProjTM

void JRenderServerDX9::SetWorldTM( const Mat4& tm )
{
    m_WorldTM = tm;
    m_pDevice->SetTransform( D3DTS_WORLD, (D3DMATRIX*)&tm );
} // JRenderServerDX9::SetProjTM

void JRenderServerDX9::SetProjTM( const Mat4& tm )
{
    m_ProjTM = tm;
    m_pDevice->SetTransform( D3DTS_PROJECTION, (D3DMATRIX*)&tm );
} // JRenderServerDX9::SetProjTM

void JRenderServerDX9::SetTextureTM( const Mat4& tm )
{
    m_TextureTM = tm;
    m_pDevice->SetTransform( D3DTS_TEXTURE0, (D3DMATRIX*)&tm );
} // JRenderServerDX9::SetTextureTM

int JRenderServerDX9::GetTextureID( const char* texName )
{
...
} // JRenderServerDX9::GetTextureID

void JRenderServerDX9::SetTexture( int texID, int stage )
{
    if (texID < 0 || texID >= (int)m_Textures.size()) return;
    m_pDevice->SetTexture( stage, m_Textures[texID].m_pTexture );
    m_TexturesPerFrame++;
} // JRenderServerDX9::SetTexture

void JRenderServerDX9::DeleteTexture( int texID )
{
    if (texID < 0 || texID >= (int)m_Textures.size()) return;
    m_Textures[texID].m_pTexture->Release();
    m_Textures[texID].m_pTexture = NULL;
} // JRenderServerDX9::DeleteTexture

void JRenderServerDX9::ShowCursor( bool bShow )
{
...
} // JRenderServerDX9::ShowCursor

int JRenderServerDX9::CreateDepthStencil( const char* texName, int w, int h, DepthFormat fmt )
{
...
int JRenderServerDX9::CreateRenderTarget( const char* texName, int w, int h, ColorFormat fmt )
{                                   
    ...
    m_Textures.push_back( tex );
    return m_Textures.size() - 1;
} // JRenderServerDX9::CreateRenderTarget

bool JRenderServerDX9::SetRenderTarget( int texID, int dsID )
{
    ...
    return (res == S_OK);
} // JRenderServerDX9::SetRenderTarget

bool JRenderServerDX9::SaveTexture( int texID, const char* fname )
{
    ...
    return (hRes == S_OK);
} // JRenderServerDX9::SaveTexture

void JRenderServerDX9::SetAmbient( uint32_t color )
{
    m_Ambience = color;
} // JRenderServerDX9::SetAmbient

int JRenderServerDX9::AddPointLight( const Vec3& pos, float radius, float falloff, 
                          uint32_t diffuse, uint32_t specular, bool bPerPixel )
{
    return 0;
} // JRenderServerDX9::AddPointLight

int JRenderServerDX9::AddDirLight( const Vec3& dir, uint32_t diffuse, uint32_t specular, bool bPerPixel )
{
    D3DLIGHT9 light;
    ...
    return 0;
} // JRenderServerDX9::AddDirLight

int JRenderServerDX9::GetConstantID( int shaderID, const char* name )
{
    ...
    return res;
} // JRenderServerDX9::GetConstantID

bool JRenderServerDX9::SetConstant( int shaderID, int constID, const Mat4& val )
{
    ...
    return (hRes == S_OK);
} // JRenderServerDX9::SetConstant

bool JRenderServerDX9::SetConstant( int shaderID, int constID, float x, float y, float z, float w )
{
    if (shaderID < 0 || shaderID >= m_Shaders.size() || constID == NULL) return false;
    const ShaderInstance& shader = m_Shaders[shaderID];
...
bool JRenderServerDX9::SetConstant( int shaderID, int constID, float val )
{
    if (shaderID < 0 || shaderID >= m_Shaders.size() || constID == NULL) return false;
    const ShaderInstance& shader = m_Shaders[shaderID];
...
int JRenderServerDX9::GetShaderID( const char* name, bool bReload )
{
    int shID = -1;
    ...
    return shID;
} // JRenderServerDX9::GetShaderID

void JRenderServerDX9::SetShader( int shID )
{
    ...
    m_ShadersPerFrame++;
} // JRenderServerDX9::SetShader

void JRenderServerDX9::SetWireFrame( bool bWireFrame ) 
{ 
    m_bWireFrame = bWireFrame; 
    m_pDevice->SetRenderState( D3DRS_FILLMODE, m_bWireFrame ? D3DFILL_WIREFRAME : D3DFILL_SOLID );
}

int JRenderServerDX9::CreateTexture( const TextureProperties& texProp )
{
    if (m_pDevice == NULL)
    {
        return -1;
    ...
    return texID;
} // JRenderServerDX9::CreateTexture

BYTE* JRenderServerDX9::LockTexture( int texID, const Frame& rect, int level, uint32_t* pStride )
{
...
} // JRenderServerDX9::LockTexture

bool JRenderServerDX9::UnlockTexture( int texID, int level )
{
    if (texID < 0 || texID >= (int)m_Textures.size()) return false;
    DXTexture* pTex = (DXTexture*)m_Textures[texID].m_pTexture;
    HRESULT hRes = pTex->UnlockRect( level );
    return (hRes == S_OK);
} // JRenderServerDX9::UnlockTexture

void JRenderServerDX9::SetColorTint( uint32_t color )
{
    m_pDevice->SetRenderState( D3DRS_TEXTUREFACTOR, color );
} // JRenderServerDX9::SetColorTint

int JRenderServerDX9::GetVDeclID( const VertexDeclaration& vdecl )
{
    if (!m_bInited) 
    {
        return -1;
...
} // JRenderServerDX9::GetVDeclID

int JRenderServerDX9::CreateVB( const char* name, int numBytes, bool bStatic )
{
    ...
    vb.m_Size           = numBytes;
    return vbID;
} // JRenderServerDX9::CreateVB

int JRenderServerDX9::GetVBID( const char* name )
{
    for (int i = 0; i < m_VBuffers.size(); i++)
    {
    ...
    return -1;
} // JRenderServerDX9::GetVBID

bool JRenderServerDX9::UnlockVB( int vbID )
{
    ...
    return (hRes == S_OK);
} // JRenderServerDX9::UnlockVB

bool JRenderServerDX9::SetVB( int vbID, int vdeclID, int stream )
{
    ...
    return (hRes == S_OK);
} // JRenderServerDX9::SetVB

bool JRenderServerDX9::DeleteVB( int vbID )
{
    if (vbID < 0 || vbID >= m_VBuffers.size()) return false;
    m_VBuffers[vbID].m_pBuffer->Release();
    m_VBuffers[vbID].m_pBuffer = NULL;
    return true;
} // JRenderServerDX9::DeleteVB

int JRenderServerDX9::CreateIB( const char* name, int numBytes, bool bStatic )
{
    ...
    ib.m_Size           = numBytes;
    return ibID;
} // JRenderServerDX9::CreateIB

int JRenderServerDX9::GetIBID( const char* name )
{
    for (int i = 0; i < m_IBuffers.size(); i++)
    {
    ...
    return -1;
} // JRenderServerDX9::GetIBID

bool JRenderServerDX9::UnlockIB( int ibID )
{
    ...
    HRESULT hRes = pBuffer->Unlock();
    return (hRes == S_OK);
} // JRenderServerDX9::UnlockIB

bool JRenderServerDX9::SetIB( int ibID, int baseIndex )
{
    if (ibID < 0 || ibID >= m_IBuffers.size()) return false;
    IDirect3DIndexBuffer9* pBuffer = m_IBuffers[ibID].m_pBuffer;
    ...
    return (hRes == S_OK);
} // JRenderServerDX9::SetIB

bool JRenderServerDX9::DeleteIB( int ibID )
{
    if (ibID < 0 || ibID >= m_IBuffers.size()) return false;
    ...
    return true;
    return false;
} // JRenderServerDX9::DeleteIB

bool JRenderServerDX9::Draw( int firstIdx, int numIdx, int firstVert, int numVert, PrimitiveType primType )
{
    ...
    return (hRes == S_OK);
} // JRenderServerDX9::Draw

int JRenderServerDX9::ClearVB( int vbID )
{
    if (vbID < 0 || vbID >= m_VBuffers.size()) return false;
    IDirect3DVertexBuffer9* pBuffer = m_VBuffers[vbID].m_pBuffer;
    if (!pBuffer) return false;
    ...
    return -1;
} // JRenderServerDX9::ClearVB

int JRenderServerDX9::ClearIB( int ibID )
{
...
} // JRenderServerDX9::ClearIB

int JRenderServerDX9::GetVBIteration( int vbID )
{
    if (vbID < 0 || vbID >= m_VBuffers.size()) return -1;
    return m_VBuffers[vbID].m_CurIteration;
} // JRenderServerDX9::GetVBIteration

int JRenderServerDX9::GetIBIteration( int ibID )
{
    if (ibID < 0 || ibID >= m_IBuffers.size()) return -1;
    return m_IBuffers[ibID].m_CurIteration;
} // JRenderServerDX9::GetIBIteration

BYTE* JRenderServerDX9::LockIB( int ibID, int firstByte, int numBytes )
{
    ...
    return (BYTE*)pData;
} // JRenderServerDX9::LockIB

bool JRenderServerDX9::CacheIB( int ibID, BYTE* pData, int size, int& iteration, int& firstByte )
{
    if (ibID < 0 || ibID >= m_IBuffers.size()) return -1;
    IBInstance& ib = m_IBuffers[ibID];
    ...
    return true;
} // JRenderServerDX9::CacheIB

BYTE* JRenderServerDX9::LockVB( int vbID, int firstByte, int numBytes )
{
    ...
    return (BYTE*)pData;
} // JRenderServerDX9::LockVB

bool JRenderServerDX9::CacheVB( int vbID, BYTE* pData, int size, int stride, int& iteration, int& firstByte )
{
    if (vbID < 0 || vbID >= m_VBuffers.size()) return -1;
    VBInstance& vb = m_VBuffers[vbID];
    ...
    return true;
} // JRenderServerDX9::CacheVB

bool JRenderServerDX9::CopyTexture( int destID, int srcID, const Frame* rct, int nRect ) 
{ 
    ...
    return (hRes == S_OK);
} //JRenderServerDX9::CopyTexture

bool JRenderServerDX9::GetTextureProp( int texID, TextureProperties& texProp )
{
    if (texID < 0 || texID >= m_Textures.size()) return false;
    texProp = m_Textures[texID].m_Prop;
    return true;
} // JRenderServerDX9::GetTextureProp

void JRenderServerDX9::Register( IRenderServerClient* pClient )
{
    for (int i = 0; i < m_Clients.size(); i++)
    {
...
} // JRenderServerDX9::Register

void JRenderServerDX9::Unregister( IRenderServerClient* pClient )
{
    for (int i = 0; i < m_Clients.size(); i++)
    {
...
} // JRenderServerDX9::Unregister

void JRenderServerDX9::ReleaseDefaultRes()
{
    if (false)
    {
...
void JRenderServerDX9::RecreateDefaultRes()
{
...
} // JRenderServerDX9::RecreateDefaultRes

void JRenderServerDX9::CreateQuadIB()
{
    //  create quad index buffer
    ...
    if (!pQuadIdx)
    {
        rlog.err( "JRenderServerDX9: Could not create internal quad index buffer." );
        return;
    }
    int curV = 0;
